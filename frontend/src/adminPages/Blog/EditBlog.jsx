...
function EditBlog() {
  ...
  // State initialization from blog data
  ...
  const [selectedNotes, setSelectedNotes] = useState({
    top: blog?.notes?.top?.map((note) => note._id) || [],
    middle: blog?.notes?.middle?.map((note) => note._id) || [],
    base: blog?.notes?.base?.map((note) => note._id) || [],
  });

  const [accords, setAccords] = useState(
    blog?.accords || [{ name: "", percentage: "", color: "#000000" }]
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append(
        "notes",
        JSON.stringify({
          top: selectedNotes.top,
          middle: selectedNotes.middle,
          base: selectedNotes.base,
        })
      );
      ...
      const response = await axios.put(
        `${BASE_URL}/api/perfumeBlog/updateBlog/${id}`,
        data,
        {
          headers: {
